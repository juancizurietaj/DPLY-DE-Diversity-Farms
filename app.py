...
i_methods_a = html.Div([html.Div(html.Label("¿De dónde provienen estos datos?", className="labels-white"),
                        ...
                        insects_methods,
                        html.Div(html.Img(src="assets/Base.png", height=650), style={"text-align": "center",
                                                                                     "margin-bottom": "8rem"})])
...
i_origin = html.Div([html.Br(),
                     html.Label("¿De dónde vienen los insectos?", className="labels-white",
                                style={"margin-top": "8rem"}),
                     insects_methods_5,
                     html.Div([
                         html.Div([html.P("Endémicos", className="pie-values", style={"color": color_success}),
                                   html.P("(8.11%)", className="pie-values", style={"color": color_success}),
                                   html.P("1340 individuos", className="pie-values",
                                          style={"color": color_success, "font-size": "3rem"})],
                                  className="pie-values-col"),
                         html.Div([html.P("Nativos", className="pie-values", style={"color": color_warning}),
                                   html.P("(5.23%)", className="pie-values", style={"color": color_warning}),
                                   html.P("865 individuos", className="pie-values",
                                          style={"color": color_warning, "font-size": "3rem"})],
                                  className="pie-values-col"),
                         html.Div([html.P("Introducidos", className="pie-values", style={"color": color_danger}),
                                   html.P("(41.00%)", className="pie-values", style={"color": color_danger}),
                                   html.P("6774 individuos", className="pie-values",
                                          style={"color": color_danger, "font-size": "3rem"})],
                                  className="pie-values-col"),
                         html.Div([html.P("Desconocido", className="pie-values", style={"color": color_secondary}),
                                   html.P("(45.70%)", className="pie-values", style={"color": color_secondary}),
                                   html.P("7554", className="pie-values",
...
footer_buttons = html.Div(
    [dbc.Button("INSECTOS", size="lg", outline=True, color="success", id="i-btn", n_clicks=1,
                style={"width": "50%", "height": "calc(10rem + 1vw)", "font-size": "7rem"}),
...
def update_layout(show_species, p_all_sectors, p_santa_rosa, p_el_carmen, p_occidente, p_aguacatal, p_media_luna,
    ...
    # Grouping data
    if not show_species:
        label_column = "Familia"
        df_grouped = df.groupby(["Familia", "Sector"])["Numero de individuos"].sum().reset_index()
    else:
        label_column = "Especie"
        df_grouped = df.groupby(["Especie", "Sector"])["Numero de individuos"].sum().reset_index()
    ...
    create_plants_meter_layout(data_p, "Familia", "Numero de individuos", "Nombre comun")
